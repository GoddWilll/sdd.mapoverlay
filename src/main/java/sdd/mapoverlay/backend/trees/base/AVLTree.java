...
public class AVLTree<D extends Comparable> extends BSTree<D> {
	...
	// On redéfinit la méthode insertEmpty de la classe BSTree, afin de travailler
	// avec
	...
	public void insertStatusStructureVariant(D d, double yp) {
		...
		if (isEmpty()) {
			...
			if (((Segment) getData()).xAtYp(yp) < ((Segment) d).xAtYp(yp)) {
				...
				// System.out.println("Data : " + getData() + " x : " + ((Segment)
				// getData()).xAtYp(yp)+ " = d : " + d + " x : " + ((Segment) d).xAtYp(yp)+ " at
				// yp : " + yp);
		...
		if (!isEmpty()) {
			...
			if (((Segment) getData()).xAtYp(yp) < ((Segment) d).xAtYp(yp)) {
				...
				if (!((Segment) getData()).isSameSegment((Segment) d)) {
					...
					// if (((Segment) getData()).getUpperEndPoint().getXCoords() > ((Segment)
					// d).getUpperEndPoint().getXCoords()){
					// getLeft().suppressStatusStructure(d, yp);
					// } else {
					// getRight().suppressStatusStructure(d, yp);
					// }
					...
					if (((Segment) getData()).xAtYp(yp) == ((Segment) d).xAtYp(yp) && height() > 2
							&& getRight().height() >= 2) {
						D data = getData();
						suppressRoot();
						D newData = getData();
	...
	/**
	 * Fonction permettant de remplacer un segment par un autre, lors d'une
	 * suppression. On remplace le segment le plus a droite du sous arbre gauche par
	 * la nouvelle valeur de la racine.
	 * 
	...
	public void replace(D d, D replacement, double yp) {
		// System.out.println("data : " + getData() + " tobereplaced : " + d + " yp : "
		// + yp);
		if (((Segment) getData()).xAtYp(yp) == ((Segment) d).xAtYp(yp)) {
			// System.out.println("data : " + getData() + " d : " + d + " replacement : " +
			// replacement + " yp : " + yp);
			if (((Segment) getData()).isSameSegment((Segment) d)) {
				setData(replacement);
			} else {
			...
			// System.out.println(((Segment) getData()).xAtYp(yp) + " < " + ((Segment)
			// replacement).xAtYp(yp));
			getRight().replace(d, replacement, yp);
		} else if (((Segment) getData()).xAtYp(yp) > ((Segment) replacement).xAtYp(yp)) {
			// System.out.println(((Segment) getData()).xAtYp(yp) + " > " + ((Segment)
			// replacement).xAtYp(yp));
